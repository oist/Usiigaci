...
def cell_tracking_clicked(param):
    ...
    with pg.BusyCursor():
        ...
        for index, row in trj.sort_values('frame').iterrows():
            ...
            i_frame = int(row['frame'])
            logging.info('Frame {}, cell {}...'.format(i_frame, cell_id))
        ...
        v_mask.setColorMap(pg.ColorMap(pos=np.linspace(0.0, 1.0, len(cell_ids)), color=color_list))
        v_mask.setCurrentIndex(current_frame_index)
