...
def prepare_mask_colors(merged_masks, trj):
    ...
    color_list = generate_distinct_colors(
        n_colors=len(cell_ids),
        n_intensity_levels=max(3, np.ceil(len(cell_ids) / 255)),  # More cells need more intensity levels
        background_color=(20, 20, 20))  # Insert color for background (slightly brighter than black to see the limits)
...
def create_colorized_masks(id_masks, trj, cell_ids, background_id,
    ...
    bg_frame = np.zeros((id_masks.shape[0], id_masks.shape[1], 3), dtype=np.uint8)  # This will hold up to 2^16 colors due to using H and V (each having 8 bits) from the HSV space
    ...
    for i_frame in range(id_masks.shape[2]):
        ...
        if show_ids:
            id_frame = np.zeros((mask.shape[0], mask.shape[1], 3), dtype=np.uint8)
...
def create_colorized_tracks(id_masks, trj, cell_ids, background_id,
    ...
    colorized_tracks = []
    max_id_mask = id_masks.max()
    for i_frame in range(id_masks.shape[2]):
        logging.info('Frame {}...'.format(i_frame))
        mask = id_masks[:, :, i_frame]
        if id_masks.dtype == np.uint8:
            col_frame = np.stack((mask.copy(), mask.copy(), mask.copy()), axis=2)
        else:
            mask_8bit = (255 * (mask / max_id_mask)).astype(np.uint8)
            col_frame = np.stack((mask_8bit.copy(), mask_8bit.copy(), mask_8bit.copy()), axis=2)
        if show_ids:
        ...
        for cell_id in cell_ids:
            ...
            if show_contours:
                # Compute cell border
                cell_mask = (mask == cell_id).astype(np.uint8)  # uint8 is ok here, we use it just to find coordinates
...
def create_track_overview(id_masks, trj, cell_ids, background_id,
    ...
    if id_masks.dtype == np.uint8:
        ...
        mask_8bit = (255 * (id_masks[:, :, -1] / max_id_mask)).astype(np.uint8)
        track_overview = np.stack((mask_8bit.copy(), mask_8bit.copy(), mask_8bit.copy()), axis=2)
